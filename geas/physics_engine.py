...
from generator.latex_engine import LATEX_SOLUTION_PREFIX as lpref
from generator.latex_engine import LATEX_SOLUTION_SUFFIX as lsuff
...
class example_2_8:
	def __init__(self,*args,**kwargs):
		...
		variation = random.randint(0, len(ask) - 1)
		self.question = f"""A ball is dropped from rest at a height of {round(height.m,2)} m above the ground. {ask[variation]}"""
...
{distanceSpecific.m:.4g} &= {velocityInitial.mpers:.4g} \\times t - \\frac{{1}}{{2}} \\times {gravityMoon.mpers2:.4g} \\times t^2 \\\\
t &= {timelist} s \\\\
{lsuff}"""
...
class example_2_17:
	def __init__(self,*args,**kwargs):
		...
		self.latex_solution = f"""{lpref}
v_i^2 - 2 g h_{{max}} &= 0 \\\\
{velocityInitial.mpers:.4g} - 2 \\times {gravity.mpers2:.4g} \\times h_{{max}} \\\\
...
{height.m:.4g} + {velocityInitial.mpers:.4g} \\times t - \\frac{{1}}{{2}} \\times {gravity.mpers2:.4g} \\times t^2 &= 0\\\\
t &= {timeToGround.s:.4g} s \\\\
{lsuff}"""
...
{heightInitial.m:.4g} + {velocityInitialY.mpers:.4g} \\times t - \\frac{{1}}{{2}} {gravity.mpers2:.4g} \\times t^2 &= 0 \\\\
...
class example_3_23:
	def __init__(self,*args,**kwargs):
		...
		self.question = f"""A cord passing over an easily turned pulley (one that is both massless and frictionless) has a {mass1.kg} kg mass hanging from one end and a {mass2.kg} kg mass hanging from the other. Find the tension of the cord."""
...
class example_9_2:
	def __init__(self,*args,**kwargs):
		...
		self.question = f"""The bob of a pendulum {length.cm} cm swings through a {arcLength.cm} cm arc. Find the angle in degrees through which it swings."""

		angle = c.angle(arcLength.m / length.m, 'rad')

		self.answer = f"""{round(angle.deg,2)} degrees"""
...
class example_9_3:
	def __init__(self,*args,**kwargs):
		...
		self.question = f"""A fan turns at a rate of {omega.rpm} rpm. Find the angular speed of any point on one of the fan blades. Find the tangential speed of the tip of a blade if the distance from the center to the tip is {distance.cm} cm."""

		tangentialSpeed = c.velocity( omega.radpers * distance.m)

		self.answer = f"""{round(tangentialSpeed.mpers,2)} m/s"""
...
class example_9_4:
	def __init__(self,*args,**kwargs):
		...
		self.answer = f"""{round(omega.radpers,2)} rad/s"""
...
class example_9_5:
	def __init__(self,*args,**kwargs):
		...
		self.question = f"""A wheel of {radius.cm} cm radius rotates on a stationary axle. It is uniformly speeded up from rest to a speed of {omegaF.rpm} rpm in a time of {time.s}s. Find the tangential acceleration of a point on its rim."""

		alpha = c.angularAcceleration((omegaF.radpers)/(time.s))
		tangentialAcceleration = c.acceleration( radius.m * alpha.radpers2)

		self.answer = f"""{round(tangentialAcceleration.mpers2,2)} m/s2"""
...
class example_9_6:
	def __init__(self,*args,**kwargs):
		...
		self.question = f"""A pulley of {radius.cm}-cm radius, on a motor, is turning at {omegaI.revpers} rev/s and slows down uniformly to {omegaF.revpers} rev/s in {time.s} s. Calculate the length of belt it winds in this time."""
		...
		self.answer =f"""{round(length.m,2)} m"""
...
class example_9_7:
	def __init__(self,*args,**kwargs):
		...
		self.answer = f"""{round(theta.rev,2)} rotations"""
...
class example_9_8:
	def __init__(self,*args,**kwargs):
		...
		self.question = f"""The spin drier of a washing machine revolving at {omegaI.rpm} rpm slows down uniformly to {omegaF.rpm} rpm while making {theta.rev} revolutions. Find the time required to turn through these {theta.rev} revolutions."""
...
class example_9_9:
	def __init__(self,*args,**kwargs):
		...
		self.question = f"""A {mass.g}g object is tied to the end of a cord and whirled in a horizontal circle of radius {radius.m} m at a constant {omega.revpers} rev/s. Assume that the cord is horizontal , i.e, that the gravity can be neglected. Determine the tension in the cord."""
...
class example_9_11:
	def __init__(self,*args,**kwargs):
		...
		self.question = f"""A spaceship orbits the Moon at a height of {height.m} m. Assuming it to be subject only to the gravitational pull of the Moon, find the time it takes for one orbit. For the Moon, mass = {massMoon.kg} kg, and radius = {radiusMoon.m} m."""
		...
		self.answer = f"""{round(time.min,2)} min"""
...
class example_9_14:
	def __init__(self,*args,**kwargs):
		...
		self.question = f"""A {mass.kg} kg body attached to a cord is whirled in a vertical circle of radius {radius.m} m. If the body has the minimum speed it must have at the top of the circle so as not to depart from the circular path, find the tension in the cord when the body is at the bottom of the circle."""
...
class example_9_16:
	def __init__(self,*args,**kwargs):
		...
		self.answer = f"""{round(omega.revpers,2)} rev/s"""
