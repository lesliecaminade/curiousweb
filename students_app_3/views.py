...
class StudentFailEnrollment(TemplateView):
    template_name = 'students_app_3/enrollment_fail.html'

class StudentListViewAll(PermissionRequiredMixin, ListView):
    permission_required = ('students_app_3.view_student')
...
class StudentDetailView(View):
    def get(self, *args, **kwargs):
        ...
        template_name = 'students_app_3/student_detail.html'
        return render(self.request, template_name, context)

class StudentCreateView(CreateView):
    model = models.Student
    form_class = forms.StudentForm
    success_url = reverse_lazy('students_app_3:success')
    ...
    def form_valid(self, form):
        try:
            ...
            r = r.json()
            if r['success'] == 'false':
                return HttpResponseRedirect(reverse('students_app_3:fail'))
...
class StudentUpdateView(PermissionRequiredMixin, UpdateView):
    permission_required = ('students_app_3.change_student')
    template_name = 'students_app_3/student_update.html'
    fields = '__all__'
    model = models.Student
    success_url = reverse_lazy("students_app_3:list_all")

class StudentDeleteView(PermissionRequiredMixin, DeleteView):
    permission_required = ('students_app_3.delete_student')
...
class StudentActivateView(View):
    def get(self, *args, **kwargs):
        if self.request.user.is_staff:
            ...
            return HttpResponseRedirect(reverse('students_app_3:detail', kwargs ={'pk': pk}))
