...
class sadiku_1_1:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{charge.C:4.4} C"""

class sadiku_1_2:
	def __init__(self,*args,**kwargs):
		#print('1_2')
		time = c.time(ran.main(0.5))
		charge = ran.main(5, 'int') * t * sympy.sin(ran.main(4, 'int') * sympy.pi * t) 
		
		self.question = f"""The total charge entering a terminal is given by 
q = {sympy.pretty(charge)} mC. Calculate the current at t = {time.ms} ms"""

		current = sympy.diff(charge, t)
		...
		self.answer =f"""{current_at_t.mA:4.4} mA"""
...
{sympy.pretty(current_function)} A."""

		charge = c.charge(sympy.integrate(
...
class sadiku_1_5:
	def __init__(self,*args,**kwargs): 
		...
		current_function = ran.main(5, 'int') * sympy.cos(ran.main(60, 'int') * sympy.pi * t)
...
and the voltage is a) {factor} i ,  b) {factor} di / dt."""
		...
		self.answer = f"""{power_a.W:4.4} W, {power_b.W:4.4} W"""
...
class sadiku_1_6:
	def __init__(self,*args,**kwargs):
		...
		self.answer = f"""{energy.Wh:4.4} Wh"""
...
class sadiku_1_7:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{p1.W:4.4} W, {p2.W:4.4} W, {p3.W:4.4} W, {p4.W:4.4} W"""
...
class sadiku_1_9:
	def __init__(self,*args,**kwargs): 
		...
		while regen:
			...
			base = c.pesos(ran.main(12))
			rate1 = c.pesos(ran.main(16)/100)
			rate2 = c.pesos(ran.main(10)/100)
			rate3 = c.pesos(ran.main(6)/100)
...
Base monthly charge of {base.pesos_string}
First 100 kWh per month at {rate1.pesos_string} per kWh.
Next 200 kWh per month at {rate2.pesos_string} per kWh.
Over 300 kWh per month at {rate3.pesos_string} per kWh."""
		...
		cost = c.pesos(
		base.pesos + rate1.pesos * 100 + rate2.pesos * 200 + rate3.pesos * (consumed.kWh - 300)
		)
		
		self.answer = f"""{cost.pesos_string}"""
...
class sadiku_2_1:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{resistance.ohms:4.4} ohms"""
...
class sadiku_2_2:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{i1.mA:4.4} mA, {g1.mS:4.4} mS, {p1.mW:4.4} mW"""

class sadiku_2_3:
	def __init__(self,*args,**kwargs): 
		
		voltage_function = ran.main(20, 'int') * sympy.sin(sympy.pi * t)
		r1 = c.resistance(ran.main(5), 'kohms')
		
		self.question = f"""A voltage source of 
{sympy.pretty(voltage_function)}
(volts) is connected across a {r1.kohms} kohms resistor. Find the current through the resistor and the power dissipated.""" 

		i1 = voltage_function / r1.ohms
		
		p1 = voltage_function * i1
		
		
		self.answer = f"""{sympy.pretty(i1 * 1000)} mA, {sympy.pretty(p1 * 1000)} mW"""
...
class sadiku_2_5:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{v1.V:4.4} V, {v2.V:4.4} V"""
...
class sadiku_2_6:
	def __init__(self,*args,**kwargs):
		...
		i_set = sympy.solveset(equation, x)
		i_list = list(i_set)
		...
		self.answer = f"""{i1.A:4.4} A, {vo.V:4.4} V"""
...
class sadiku_2_7:
	def __init__(self, *args, **kwargs):
		...
		io_set = sympy.solveset(equation, x)
		io_list = list(io_set)
		...
		self.answer = f"""{io.A:4.4} A, {vo.V:4.4} V"""
...
class sadiku_2_8:
	def __init__(self, *args, **kwargs):
		...
		equations = [
		sympy.Eq(x - y - z, 0),
		sympy.Eq(- vs.V + r1.ohms * x + r2.ohms * y, 0),
		sympy.Eq(r3.ohms * z , r2.ohms * y)
		]
		
		solutions_set = sympy.linsolve(equations, x, y, z)
		#print(solutions_set)
		...
		self.answer = f"""{i1.A:4.4} A, {i2.A:4.4} A, {i3.A:4.4} A, {v1.V:4.4} V, {v2.V:4.4} V, {v3.V:4.4} V"""
...
class johnbird_2_3:
	def __init__(self,*args,**kwargs): 
		...
		self.question = f"""The current flowing through a resistor is {current.A:4.4} A when a p.d. of {voltage.V:4.4} V is applied. Determine the value of the resistance."""
		
		resistance = c.resistance(voltage.V / current.A)
		
		self.answer = f"""{resistance.ohm:4.4} ohms"""
...
class johnbird_2_6:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{resistance.ohm:4.4} ohms, {current2.mA:4.4} mA"""
...
class johnbird_2_7:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{resistance.ohm:4.4} ohms, {resistance2.ohm:4.4} ohms"""
...
class johnbird_2_9:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{current.A:4.4} A, {resistance.ohm:4.4} ohms"""
...
class johnbird_2_11:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{current.A:4.4}  A, {power.kW:4.4} kW"""
...
class johnbird_2_12:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{voltage.V:4.4} V, {power.kW:4.4} kW"""
...
class johnbird_2_13:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{current.A:4.4} A, {power.W:4.4}  W"""
...
class johnbird_2_14:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{current.A:4.4} A, {power.W:4.4} W, {energy.J:4.4} J"""
...
class johnbird_2_15:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{energy.kJ:4.4} kJ"""
...
class johnbird_2_16:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""P {costperweek:4.4}"""
...
class johnbird_2_17:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{power.kW:4.4} kW, {current.A:4.4} A"""
...
class johnbird_3_1:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{resistance2.ohms:4.4} ohms, {length3.m:4.4} meters"""
...
class johnbird_3_2:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{resistance2.ohms:4.4} ohms, {area3.mm2:4.4} mm2"""
...
class johnbird_3_4:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{resistance.ohm:4.4} ohm"""
...
class johnbird_3_5:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{area.mm2:4.4} mm2"""
...
class johnbird_3_6:
	def __init__(self,*args,**kwargs): 
		...
		self.answer = f"""{resistivity.uohm_m:4.4} uohm - m"""
...
class johnbird_3_8:
	def __init__(self,*args,**kwargs): 
		...
		self.question = f"""A coil of copper wire has a resistance of {resistance.ohms} ohms when its temperature is {temperature.C} degC. Determine its resistance at {temperature2.C} degC if the temperature coefficient of resistance of copper at {temperature.C} degC is {coefficient} per degC."""
		
		resistance2 = c.resistance(resistance.ohms + resistance.ohms * coefficient * (temperature2.C - temperature.C))
...
class johnbird_3_12:
	def __init__ (self):
		...
		equation = sympy.Eq(resistance2.ohms, resistance1.ohms * ( 1 + coe * (x - temp1.C) ) )
		...
		self.answer = f"""{temp2.C:4.4} degC"""
...
class johnbird_3_14:
	def __init__(self):
		...
		self.answer = f"""{resistor1.resistance.ohms} ohms, {resistor1.tolerance}"""

class johnbird_3_16:
	def __init__(self):
		resistor1 = c.resistor(band = random.randint(3,4))
		self.question = f"""Between what two values should a resistor with color coding: {resistor1.color} lie?"""
		self.answer = f"""{resistor1.minresistance.ohms} ohms to {resistor1.maxresistance.ohms:4.4} ohms"""
...
class johnbird_3_17:
	def __init__(self):
		...
		self.answer = f"""{resistor1.color:4.4} """
...
class johnbird_3_19:
	def __init__(self):
		...
		j = random.randint(0,len(prefix_values) - 1)
		...
		self.question = f"""Determine the value of a resistor marked as {D1}{prefix_list[j]}{D0}{tolerance_code}."""

		self.answer = f"""{resistance1.ohms:4.4} ohms (+/-) {tolerance}"""
...
class johnbird_5_1:
	def __init__(self):
		...
		self.answer = f"""{rt.ohms:4.4} ohms"""
...
class johnbird_5_2:
	def __init__(self):
		...
		self.answer = f"""{r2.ohms:4.4} ohms"""
...
class johnbird_5_5:
	def __init__(self):
		...
		self.answer = f"""{energy.kWh:4.4} kWh"""
...
class johnbird_5_16:
	def __init__(self):
		...
		self.answer = f"""{r1.ohms:4.4} ohms"""
...
class johnbird_6_4:
	def __init__(self):
		...
		self.answer = f"""{electricfield.kVperm:4.4} kV/m"""
...
class johnbird_6_8:
	def __init__(self):
		...
		self.question = f"""A waxed paper capacitor has two parallel plates, each of effective area {area.cm2:4.4} cm2. If the capacitance of the capacitor is {capacitance.pF} pF determine the effective thickness of the paper if its    relative permittivity is {er}."""
		self.answer = f"""{spacing.mm:4.4} mm"""
...
class johnbird_6_9:
	def __init__(self):
		...
		self.answer = f"""{capacitance.nF:4.4} nF"""
...
class johnbird_6_16:
	def __init__(self):
		...
		self.answer = f"""{power.kW:4.4} kW"""

class johnbird_6_17:
	def __init__(self):
		capacitance = c.capacitance(ran.main(12),'uF')
		energy = c.energy(ran.main(4))
		voltage = c.voltage( math.sqrt(2 * energy.J / capacitance.F ))
...
class johnbird_6_18:
	def __init__(self):
		...
		self.answer = f"""{capacitance.uF:4.4} uF"""

class johnbird_7_1:
	def __init__(self):
		length = c.length(ran.main(200), 'mm')
		width = c.length(ran.main(100), 'mm')
		flux = c.magneticFlux(ran.main(150), 'uWb')
		area = c.area(length.m * width.m)
		fluxdensity = c.magneticFluxDensity(flux.Wb / area.m2)

		self.question = f"""A magnetic pole face has a rectangular section having dimensions {length.mm} mm by {width.mm} mm. If the total flux emerging from the pole is {flux.uWb} uWb, calculate the flux density."""
		self.answer = f"""{fluxdensity.mT:4.4} mT"""
...
class johnbird_7_2:
	def __init__(self):
		...
		self.answer = f"""{radius.mm:4.4} mm"""
...
class johnbird_7_4:
	def __init__(self):
		...
		self.answer = f"""{ur:4.4}"""

class johnbird_7_5:
	def __init__(self):
		B = c.magneticFluxDensity(ran.main(0.25), 'T')
...
class johnbird_7_6:
	def __init__(self):
		...
		self.answer = f"""{flux.uWb:4.4} uWb"""
...
class johnbird_7_7:
	def __init__(self):
		...
		fluxdensity = c.magneticFluxDensity(ran.main(0.4))
		length = c.length( math.pi * diameter.m )
...
class johnbird_7_8:
	def __init__(self):
		...
		self.answer = f"""{reluctance.perH:4.4} /H"""
...
class johnbird_7_10:
	def __init__(self):
		...
		fluxdensity = c.magneticFluxDensity(flux.Wb / area2.m2)

		self.question = f"""A closed magnetic circuit of cast steel contains a {length1.cm} cm long path of cross sectional area {area1.cm2} cm2 and a {length2.cm} cm path of cross-sectional area {area2.cm2} cm2. A coil of {turns} turns is wound around the {length1.cm} cm length of the circuit and a current of {current.A} A flows. Determine the flux density in the {length2.cm} cm path if the relative permeabilty of cast steel is {ur}.""" 
		self.answer = f"""{fluxdensity.T:4.4} T"""
...
class johnbird_8_4:
	def __init__(self):
		...
		flux = c.magneticFlux(ran.main(0.5), 'mWb')
...
class johnbird_9_1:
	def __init__(self):
		...
		resistance = c.resistance(ran.main(20))
		velocity = c.velocity(ran.main(10), 'mpers')
		...
		self.answer = f"""{current.mA:4.4} mA"""
...
class johnbird_9_2:
	def __init__(self):
		...
		self.answer = f"""{velocity.mpers:4.4} m/s"""
...
class johnbird_9_3:
	def __init__(self):
		...
		self.answer = f"""{voltage.mV:4.4} mV"""
...
class johnbird_9_8:
	def __init__(self):
		...
		self.answer = f"""{voltage.V:4.4} V"""
		self.question = f"""A flux of {flux.uWb} uWb passing through a {turns}-turn coil is reversed in {time.ms} ms. Find the average emf induced."""
...
class johnbird_9_10:
	def __init__(self):
		...
		self.answer = f"""{inductance.H:4.4} H"""
...
class johnbird_9_17:
	def __init__(self):
		...
		inductance = c.inductance( turns**2 / reluctance.perH )
		voltage = c.voltage( - inductance.H * current.A / time.s )
		self.answer = f"""{voltage.V:4.4} V"""
...
class johnbird_9_20:
	def __init__(self):
		...
		fluxchange = c.magneticFlux( abs(voltage.V ) * time.s / turns)
		self.answer = f"""{fluxchange.mWb:4.4} mWb"""
...
class johnbird_10_1:
	def __init__(self):
		...
		self.answer = f"""{rshunt.mohms:4.4} mohms"""
...
class johnbird_10_2:
	def __init__(self):
		...
		self.answer = f"""{rseries.kohms:4.4} kohms"""
...
class johnbird_10_4:
	def __init__(self):
		...
		self.answer = f"""{meter_reading.mA:4.4} mA""" 
...
class johnbird_10_18:
	def __init__(self):
		...
		self.answer = f"""{rx.ohms:4.4} ohms"""
...
class johnbird_10_20:
	def __init__(self):
		...
		self.answer = f"""{inductance.H:4.4} H, {resistance.ohms:4.4} ohms"""
...
class johnbird_10_21:
	def __init__(self):
		...
		inductance = c.inductance( 1 / ( (2 * math.pi * frequency.Hz)**2 * capacitance.F ))
		resistance = c.resistance( 2 * math.pi * frequency.Hz * inductance.H / Q)

		self.answer = f"""{inductance.uH:4.4} uH, {resistance.ohms:4.4} ohms"""
...
class johnbird_10_22:
	def __init__(self):
		resistor = c.resistance( ran.main(5), 'kohms' )
		...
		self.answer  = f"""{voltage.V:4.4} (+/-) {voltage_error.V:4.4} V"""
...
class johnbird_10_23:
	def __init__(self):
		...
		self.answer = f"""{resistance.ohms:4.4} (+/-) {resistance_error.ohms:4.4} ohms"""
...
class johnbird_10_24:
	def __init__(self):
		...
		self.answer = f"""{resistancex.ohms:4.4} (+/-) {resistancex_error.ohms:4.4} ohms"""
