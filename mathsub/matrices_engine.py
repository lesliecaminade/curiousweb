...
class Matrix_Function():
	def __init__(self, *args, **kwargs):
		...
		mat_i = Matrix()
		mat_i.init_identity(rows = dim, cols = dim)

		result = a * mat_a.multiply(mat_a.multiply(mat_a)).matrix + b * mat_a.multiply(mat_a).matrix + c * mat_a.matrix + d * mat_i.matrix

		function = a * x**3 + b * x**2 + c * x + d
		self.question = f"""If f(x) = {parse(function)}, determine the value of f(A) is A = 
{mat_a.matrix}"""
